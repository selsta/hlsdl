		...
		while ((src = (strchr(src, '\n')))) {
			...
			if (end_ptr != NULL) {
				...
				if (me->encryptiontype == ENC_AES128 || me->encryptiontype == ENC_AES_SAMPLE) {
					...
					if (hls_args.key_value && hls_args.key_value != NULL)
						memcpy(ms->enc_aes.key_value, hls_args.key_value, KEYLEN);
...
//{
//
//	MSG_VERBOSE("Downloading segment.\n");
//	MSG_API("{\"d_t\":\"vod\"}\n"); // d_t - download type
...
//				{
//					//download_size += 
//						out_ctx->write(seg.data, seg.len, out_ctx->opaque);
//					fclose(out_file);
//				}
//			}
//
...
int fill_key_value(struct enc_aes128* es)
{
	...
	else if (NULL != hls_args.key_value && hls_args.key_value)
	{
